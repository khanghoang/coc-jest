...
const checkCreateReactApp = async () => {
  ...
  const defaultCreateReactPath =
    "node_modules/react-scripts/node_modules/.bin/jest";
  const defaultCreateReactPathWindows =
    "node_modules/react-scripts/node_modules/.bin/jest.cmd";

  for (let p of [defaultCreateReactPathWindows, defaultCreateReactPath]) {
    let exists = await existAsync(path.join(root, p));
    if (exists) {
      return "npm test --";
    }
  }
