...
export class TestResultProvider {
  ...
  getResults(filePath: string): TestResult[] {
    ...
    const matchTests = (
      ...
      const _trackRemaining =
        trackRemaining === undefined ? true : trackRemaining;

      _itBlocks.forEach((test) => {
        const matched = remainingAssertions.filter((a) =>
          _isMatched.every((m) => m(test, a))
        );
        if (matched.length === 1) {
    ...
    const matchPos = (t: ItBlock, a: TestAssertionStatus): boolean =>
      (a.line !== undefined &&
        ...
        a.line <= t.end.line) ||
      (a.location &&
        a.location.line >= t.start.line &&
        a.location.line <= t.end.line);

    const matchName = (t: ItBlock, a: TestAssertionStatus): boolean =>
      t.name === a.title;
    const templateLiteralPattern = /\${.*?}/; // template literal pattern
    const matchTemplateLiteral = (
      t: ItBlock,
    ...
    const onMatchError: OnMatchError = (
      t: ItBlock,
      match: TestAssertionStatus[]
    ) => {
      let err: string;
    ...
    if (assertions.length > 0 && itBlocks.length > 0) {
      ...
      for (const [matchers, onError] of algorithms) {
        ...
        [result, itBlocks, assertions] = matchTests(
          itBlocks,
          assertions,
          matchers,
          onError
