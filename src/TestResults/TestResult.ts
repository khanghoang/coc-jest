 ...
 */
export function resultsWithLowerCaseWindowsDriveLetters(
  ...
  if (path.sep === "\\") {
    return {
      ...
      testResults: testResultsWithLowerCaseWindowsDriveLetters(
        data.testResults
      ),
    };
  }
...
export function coverageMapWithLowerCaseWindowsDriveLetters(
  data: JestTotalResults
) {
  if (!data.coverageMap) {
    return;
...
function testResultWithLowerCaseWindowsDriveLetter(
  testResult: JestFileResults
): JestFileResults {
...
export function withLowerCaseWindowsDriveLetter(
  filePath: string
): string | undefined {
